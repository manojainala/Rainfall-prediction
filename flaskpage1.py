...
def visualization():
    if request.method == 'POST':
        ...
        yearlyRainfall = locationdf.groupby('year')['RISK_MM'].mean()
        
        yearlyRainfall.plot.bar(figsize=(23,12), fontsize = 22,legend=False)
        plt.suptitle('Mean Yearly Rainfall at '+results['Location'], fontsize=30)
        ...
        if len(results) >= 2:
            locationdfAndMonthly = locationdf.loc[locationdf.year == results['year']]
            piechartLocationAndYear = []
            piechartLocationAndYear.append(locationdfAndMonthly.loc[locationdfAndMonthly.RISK_MM == 0].shape[0])
            piechartLocationAndYear.append(locationdfAndMonthly.loc[(locationdfAndMonthly.RISK_MM != 0) & (locationdfAndMonthly.RISK_MM < 2.2)].shape[0])
            piechartLocationAndYear.append(locationdfAndMonthly.loc[(locationdfAndMonthly.RISK_MM >= 2.2) & (locationdfAndMonthly.RISK_MM < 6.566875342359574)].shape[0])
            piechartLocationAndYear.append(locationdfAndMonthly.loc[locationdfAndMonthly.RISK_MM >= 6.566875342359574].shape[0])
            locationdfAndMonthly['month'] = locationdfAndMonthly['Date'].str[5:7]
            locationdfAndMonthly['month'] = locationdfAndMonthly['month'].map({
            '01':'Jan',
            '02':'Feb',
            '03':'Mar',
            '04':'Apr',
            ...
            monthList = locationdfAndMonthly.month.unique()
            monthlyRainfall = locationdfAndMonthly.groupby('month')['RISK_MM'].mean()
            monthlyRainfall.plot.bar(figsize=(23,12), fontsize = 22,legend=False)
            plt.suptitle('Mean Monthly Rainfall at '+results['Location']+" in "+results['year'], fontsize=30)
            plt.xlabel('Month', fontsize=30)
            plt.ylabel('Rainfall in MM', fontsize=30)
            plt.savefig('templates/monthlyRainfall.png')
            
            if len(results) == 3:
                locationdfAndMonthlyAndDay = locationdfAndMonthly.loc[locationdfAndMonthly.month == results['month']]
                piechartLocationAndYearAndMonth = []
                piechartLocationAndYearAndMonth.append(locationdfAndMonthlyAndDay.loc[locationdfAndMonthlyAndDay.RISK_MM == 0].shape[0])
                piechartLocationAndYearAndMonth.append(locationdfAndMonthlyAndDay.loc[(locationdfAndMonthlyAndDay.RISK_MM != 0) & (locationdfAndMonthlyAndDay.RISK_MM < 2.2)].shape[0])
                piechartLocationAndYearAndMonth.append(locationdfAndMonthlyAndDay.loc[(locationdfAndMonthlyAndDay.RISK_MM >= 2.2) & (locationdfAndMonthlyAndDay.RISK_MM < 6.566875342359574)].shape[0])
                piechartLocationAndYearAndMonth.append(locationdfAndMonthlyAndDay.loc[locationdfAndMonthlyAndDay.RISK_MM >= 6.566875342359574].shape[0])
                locationdfAndMonthlyAndDay['day'] = locationdfAndMonthlyAndDay['Date'].str[8:10]
                dailyRainfall = locationdfAndMonthlyAndDay.groupby('day')['RISK_MM'].mean()
                locationdfAndMonthlyAndDay.plot.bar(x="day",y="RISK_MM", fontsize = 22,figsize=(23,12),legend=False)
                plt.suptitle('Daily Rainfall at '+results['Location']+" in "+results['month']+" "+results['year'], fontsize=30)
                plt.xlabel('Day', fontsize=30)
                plt.ylabel('Rainfall in MM', fontsize=30)
                plt.savefig('templates/dailyRainfall.png')
                return render_template('/visualization.html',piechartLocationAndYearAndMonth=piechartLocationAndYearAndMonth,piechartLocationAndYear=piechartLocationAndYear,piechartLocation=piechartLocation,dailyRainfall=1,totalMonths=len(monthList),monthList=monthList,year=results['year'],monthlyRainfall=1,Location=results['Location'],yearlyRainfall=1,randNum=random.randint(0, 99999999999),yearList=yearList,totalYears=len(yearList) )
            else:
                return render_template('/visualization.html',piechartLocationAndYear=piechartLocationAndYear,piechartLocation=piechartLocation,dailyRainfall=0,totalMonths=len(monthList),monthList=monthList,year=results['year'],monthlyRainfall=1,Location=results['Location'],yearlyRainfall=1,randNum=random.randint(0, 99999999999),yearList=yearList,totalYears=len(yearList) )
        else:
            return render_template('/visualization.html',piechartLocation=piechartLocation,monthlyRainfall=0,Location=results['Location'],yearlyRainfall=1,randNum=random.randint(0, 99999999999),yearList=yearList,totalYears=len(yearList) )
    else:
